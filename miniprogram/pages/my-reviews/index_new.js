// pages/my-reviews/index_new.js - 我的评价（全新设计）
...
Page({
      ...
      if (res.success) {
        const list = (res.list || []).map(item => {
          ...
          return {
            ...
            reviewerAvatar: item.reviewer?.avatarUrl ? processImageUrl(item.reviewer.avatarUrl) : ''
          }
        })
  ...
  exportReviews() {
    ...
    const exportText = allReviews.map(review => {
      ...
      const target = review.reviewerName || review.reviewee?.nickName || '用户'
      const rating = review.ratingInfo?.text || '未评价'
